...
function find_user($db, $user_id)
{
  $records = exec_sql_query(
    $db,
    "SELECT * FROM reviewers WHERE id = :user_id;",
...
function password_login($db, &$messages, $username, $password)
{
  ...
  if (isset($username) && isset($password)) {
    ...
    $records = exec_sql_query(
      $db,
      "SELECT * FROM reviewers WHERE username = :username;",
...
function create_account($db, $name, $username, $password, $password_confirmation)
{
  ...
  if (empty($username)) {
    ...
    $records = exec_sql_query(
      $db,
      "SELECT username FROM reviewers WHERE (username = :username);",
      array(
        ':username' => $username
      )
    )->fetchAll();
  ...
  if ($account_valid) {
    $result = exec_sql_query(
      $db,
      "INSERT INTO reviewers(name, username, password) VALUES (:name, :username, :password);",
      array(
        ':name' => $name,
        ':username' => $username,
